...
void *setup_DHT(int dht_gpio_pin)
{
    ...
    //ISR_DATA_LIST = malloc(sizeof(isr_data) * ISR_DATA_INDEX_MAX);
...
#else
    dhtc->dht_rx_chan_config.clk_src = RMT_CLK_SRC_REF_TICK;   // select source clock
    dhtc->dht_rx_chan_config.resolution_hz = 1 * 1000 * 1000; // 1 MHz tick resolution, i.e., 1 tick = 1 µs
...
#else
    dhtc->dht_tx_chan_config.clk_src = RMT_CLK_SRC_REF_TICK;   // 1MHz
    dhtc->dht_tx_chan_config.gpio_num = dht_gpio_pin;         // GPIO number
...
int read_DHT(void *dht_handle)
{
...
#if DO_ISR == 1
    //gpio_isr_edge_handler((void *) dhtc->dht_gpio_pin);
#endif

    // Receiver setup
#if DO_ISR == 0
	ESP_ERROR_CHECK(rmt_rx_register_event_callbacks(dhtc->dht_rx_chan, &dhtc->dht_rx_cbs, dhtc->dht_rx_queue));
    dhtc->dht_rx_config.signal_range_min_ns = 10*1000;     // the shortest duration for DHT22 signal is 26 µs
    dhtc->dht_rx_config.signal_range_max_ns = 2500*1000;   // the longest duration for DHT22 signal is 5000 µs (5 ms), 10 ms max so the receive does not stop early
    ...
    int index = ((ISR_DATA_INDEX >= ISR_DATA_INDEX_MAX) ? ISR_DATA_INDEX_MAX : ISR_DATA_INDEX);
    for (int i = 0; i < index; i++) {
    	isr_data isrd = ISR_DATA_LIST[i];
    	int timeDiff = ((i == 0) ? 0 : (isrd.micros - ISR_DATA_LIST[i-1].micros));
    	// The level is the new, transition level. So a time diff of 2000 and a level of 1
    	// means it was low for 2000 micro seconds and is now high
    	printf("\t [%d][%lld] timeDiff %d, new level %d\n", i, isrd.micros, timeDiff, isrd.level);
    }
    ISR_DATA_INDEX = 0;
