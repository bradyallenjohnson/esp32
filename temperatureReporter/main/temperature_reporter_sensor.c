...
typedef struct dht_context_ {
    ...
    float humidity;
    float temperature;
    temperature_sensor_reporter reporter_func;
    void *reporter_data;

} dht_context;

// forward declaration, defined below
static bool temperature_sensor_do_reading();
                ...
                void *reporter_data)
{
    ...
    dhtc->reporter_func = reporter_func;
    dhtc->reporter_data = reporter_data;
...
void temperature_sensor_task(void *data)
{
    printf("Starting DHT Task\n\n");
    int result = 0;
    dht_context *context = (dht_context *) data;

    while(1) {
        if (temperature_sensor_do_reading()) {
            printf("=== Reading DHT ===\n" );
            result = read_DHT(context);

            error_handler(result);
            if (result == DHT_OK && context->reporter_func != NULL) {
                printf("Hum %.1f\n", get_humidity(context));
                printf("Tmp %.1f\n", get_temperature(context));
                context->reporter_func(
                        context->reporter_data,
                        get_temperature(context),
                        get_humidity(context));
            }
        }
...
bool temperature_sensor_do_reading()
{
    bool do_reading;

    taskENTER_CRITICAL(&my_spinlock);
    do_reading = do_sensor_reading;
...
{
    taskENTER_CRITICAL(&my_spinlock);
    do_sensor_reading = true;
    taskEXIT_CRITICAL(&my_spinlock);
}


// Initialize the DHT22 sensor and start the task
bool temperature_sensor_initialize(int temp_sensor_pin,
                                   temperature_sensor_reporter reporter_func,
                                   void *reporter_data)
{
    void *temp_sensor_handle =
            setup_DHT(temp_sensor_pin, reporter_func, reporter_data);

    xTaskCreate(
            &temperature_sensor_task,  // Task to execute
            "temperature_sensor_task", // Task name
            4096,                      // Stack size in words
            temp_sensor_handle,        // Task input parameter
